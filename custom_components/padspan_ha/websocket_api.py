...
def _auto_diagnostics(hass: HomeAssistant) -> Dict[str, Any]:
    ...
    checks.append(_check("domain_data_present", domain_data_present, "hass.data contains '%s'" % DOMAIN))
    if not domain_data_present:
        recommendations.append("Integration domain data missing. Restart Home Assistant after reinstall.")

    entries = hass.config_entries.async_entries(DOMAIN)
    checks.append(_check("config_entries_found", len(entries) > 0, "Found %d config entr%s" % (len(entries), "y" if len(entries) == 1 else "ies")))
    if not entries:
        recommendations.append("No PadSpan config entry found. Add integration first.")

    service_ok = hass.services.has_service(DOMAIN, SERVICE_SET_TEST_PRESENCE)
    checks.append(_check("test_service_registered", service_ok, "Service %s.%s" % (DOMAIN, SERVICE_SET_TEST_PRESENCE)))
    if not service_ok:
        recommendations.append("Service registration missing; restart HA after install.")
    ...
    if status_entries:
        ...
        checks.append(_check("entry_health", len(degraded) == 0, "%d degraded entries out of %d" % (len(degraded), len(status_entries)), level="warn"))
        if degraded:
            recommendations.append("One or more entries are degraded; check Last Error in panel.")
    else:
