...
export function render(ctx){
  ...
  function renderIsoFloorStack(){
    ...
    // ── Traceback Playback Button ──────────────────────────────────────────
    const tbBtn = document.createElement("button");
    tbBtn.className = "btn inline";
    tbBtn.style.cssText = "color:#fbbf24;border-color:#92400e";
    tbBtn.textContent = "Traceback";
    tbBtn.title = "Playback object movement history on the 3D map (NVR-style)";
    tbBtn.addEventListener("click", () => _openTracebackPanel());
    ctrlRow.appendChild(tbBtn);

    outer.appendChild(ctrlRow);
    outer.appendChild(isoWrap);
    outer.appendChild(roomListPanel);

    // ── Traceback Playback Panel ───────────────────────────────────────────
    const tbPanel = document.createElement("div");
    tbPanel.style.cssText = "display:none;margin-top:10px";

    // State for traceback
    if (!ctx.state._traceback) ctx.state._traceback = {
      active: false,
      playing: false,
      speed: 1,       // 1x, 2x, 4x, 8x, 16x, 32x
      frameIdx: 0,
      frames: [],
      range: null,     // {start, end, count}
      objKeys: [],     // available objects
      filterKey: null,  // null = all objects
      filterName: "All objects",
      rangePreset: 300, // seconds (5 min default)
      startTs: null,
      endTs: null,
      _animTimer: null,
    };
    ...
    async function _loadTracebackData() {
      try {
        ...
        tb.frameIdx = 0;
        // Also load available objects for filtering
    ...
    function _buildTracebackSVG(frameIdx) {
      // Render the base 3D map (floor slabs + rooms + receivers) then overlay playback dots
      const focusZ = _getFocusZ(ctx.state._overviewIsoFocusIdx);
      const slabWZ = 18/_ovFG;
      const maxIsoZ = sortedIsoLevels.length ? sortedIsoLevels[sortedIsoLevels.length-1] : 0;
      ...
      if (tb.frames.length && frameIdx >= 0 && frameIdx < tb.frames.length) {
        ...
        // Assign stable color per object key
        const _colorMap = {};
        let _ci = 0;
        for (const o of objs) {
        ...
        // Render trail from prev frames (faded dots showing recent path)
        ...
        for (let ti = Math.max(0, frameIdx - trailLen); ti < frameIdx; ti++) {
          ...
          for (const to of (trailFrame.o || [])) {
            const trailRoom = to.r;
            if (!trailRoom || !roomIsoPos[trailRoom]) continue;
            const tpos = roomIsoPos[trailRoom];
        ...
        for (const o of objs) {
          ...
          const tip = `${lbl} | Room: ${room}${o.rssi ? " | RSSI: "+o.rssi+" dBm" : ""}`;
    ...
    function _renderTracebackControls() {
      tbPanel.innerHTML = "";
      ...
      card.className = "card";
      card.style.cssText = "border-color:#92400e;background:#0f0a00";
      ...
      closeBtn.style.cssText = "margin-left:auto;color:#f87171;border-color:#7f1d1d;font-size:11px";
      closeBtn.textContent = "Close";
      closeBtn.addEventListener("click", () => {
        _stopPlayback();
        tb.active = false;
        _renderTracebackControls();
        // Restore live map
        isoDiv.innerHTML = buildIsoSVG(_getFocusZ(ctx.state._overviewIsoFocusIdx));
        tbBtn.style.cssText = "color:#fbbf24;border-color:#92400e";
        tbBtn.textContent = "Traceback";
      ...
      if (!tb.frames.length) {
        ...
        tbPanel.appendChild(card);
        return;
      }

      // ── NVR-style transport controls ──
      ...
      rewBtn.innerHTML = "&#9198;"; // rewind
      rewBtn.title = "Jump to start";
      rewBtn.addEventListener("click", () => { tb.frameIdx = 0; _renderTracebackFrame(); _updateScrubber(); });
      ...
      stepBackBtn.innerHTML = "&#9664;"; // prev
      stepBackBtn.title = "Previous frame";
      stepBackBtn.addEventListener("click", () => {
        if (tb.frameIdx > 0) { tb.frameIdx--; _renderTracebackFrame(); _updateScrubber(); }
      });
      ...
      playBtn.innerHTML = tb.playing ? "&#9646;&#9646;" : "&#9654;"; // pause or play
      playBtn.title = tb.playing ? "Pause" : "Play";
      playBtn.addEventListener("click", () => {
        if (tb.playing) { _stopPlayback(); }
        else { _startPlayback(); }
        _renderTracebackControls();
      });
      ...
      stepFwdBtn.innerHTML = "&#9654;"; // next
      stepFwdBtn.title = "Next frame";
      stepFwdBtn.addEventListener("click", () => {
        if (tb.frameIdx < tb.frames.length - 1) { tb.frameIdx++; _renderTracebackFrame(); _updateScrubber(); }
      });
      ...
      endBtn.innerHTML = "&#9197;"; // fast forward
      endBtn.title = "Jump to end";
      endBtn.addEventListener("click", () => { tb.frameIdx = tb.frames.length - 1; _renderTracebackFrame(); _updateScrubber(); });
      ...
      card.appendChild(scrubberWrap);
      tbPanel.appendChild(card);
    }

    function _updateScrubber() {
      const scrubber = tbPanel.querySelector('input[type="range"]');
      if (scrubber) scrubber.value = String(tb.frameIdx);
      _updateTimeLbl();
    }

    function _updateTimeLbl() {
      const timeLbl = tbPanel.querySelector("#tb-time-lbl");
      if (timeLbl && tb.frames[tb.frameIdx]) {
        timeLbl.textContent = _fmtDate(tb.frames[tb.frameIdx].ts);
      }
      const progLbl = tbPanel.querySelector("#tb-progress-lbl");
      if (progLbl) progLbl.textContent = `${tb.frameIdx + 1} / ${tb.frames.length}`;
    }

    function _renderTracebackFrame() {
      if (!tb.active || !tb.frames.length) return;
      isoDiv.innerHTML = _buildTracebackSVG(tb.frameIdx);
    }

    function _startPlayback() {
      if (tb._animTimer) clearInterval(tb._animTimer);
      tb.playing = true;
      // Base interval: ~200ms per frame at 1x; faster at higher speeds
    ...
    async function _openTracebackPanel() {
      if (tb.active) {
        // Toggle off
        _stopPlayback();
        tb.active = false;
      ...
      tb.active = true;
      tbBtn.style.cssText = "background:#92400e;color:#fbbf24;border-color:#fbbf24;font-weight:700";
      tbBtn.textContent = "Traceback ON";
      await _loadTracebackData();
      _renderTracebackControls();
      _renderTracebackFrame();
    }

    outer.appendChild(tbPanel);
    return outer;
  }
